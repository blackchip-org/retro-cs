...
I have some thoughts on [memory](memory.md). The Commodore 64 uses a [banked memory scheme](https://www.c64-wiki.com/wiki/Bank_Switching) but working out the mechanics isn't necessary at this point. Configure memory to look like the default bank for now.

### CPU

*TODO: Add 6502 notes here*

### Tools

A full blown monitor interface is nice but here is the minimum tool set required to diagnose issues that might come up:

- Memory dumper which can show text from the following character encodings:
  - PETSCII shifted
  - PETSCII unshifted
  - Screen codes shifted
  - Screen codes unshifted
- Disassembler
- Execution tracer: disassemble the instructions being executed

### First Light

The main loop is simple at this point, endlessly executing the next CPU instruction. Load in the ROMs, start the emulator, and see if video memory is being cleared properly:
...
Enable VSync, add the screen rendering, and an IRQ to the main run loop. The following is a good starting point for the loop that works well enough in the beginning:
